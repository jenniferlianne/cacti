 ...
 */
 ...
 /**
  * InteropAssetTypeStateContract 
  *
  */
 class InteropAssetTypeStateContract : Contract {
     companion object {
         // Used to identify our contract when building a transaction.
         const val ID = "org.hyperledger.cacti.weaver.imodule.corda.contracts.InteropAssetTypeStateContract"
     }
     ...
     override fun verify(tx: LedgerTransaction) {
         ...
         when (command.value) {
             is Commands.Issue -> requireThat {
                 ...
                 "The output state should be of type InteropAssetTypeState" using (tx.outputs[0].data is InteropAssetTypeState)
      ...
      /*        is Commands.Update -> requireThat {
                 ...
                 "The input state should be of type AccessControlPolicyState" using (tx.inputs[0].state.data is InteropAssetTypeState)
                 "There should be one output state" using (tx.outputs.size == 1)
                 "The output state should be of type AccessControlPolicyState" using (tx.outputs[0].data is InteropAssetTypeState)
                 val out = tx.outputsOfType<InteropAssetTypeState>().single()
     ...
     */       is Commands.Delete -> requireThat {
                 "There should be one input state" using (tx.inputs.size == 1)
                 "The input state should be of type AccessControlPolicyState" using (tx.inputs[0].state.data is InteropAssetTypeState)
                 val input = tx.inputs[0].state.data as InteropAssetTypeState
