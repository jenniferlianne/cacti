...
describe("PluginCopmFabric", () => {
  ...
  */
  ...
  test("corda-corda can lock/claim tokens on same network", async () => {
    ...
    const srcCert = await cordaTestnet.getCertificateString(u1n1);
    const destCert = await cordaTestnet.getCertificateString(u2n1);
    ...
    const lockResult = await client.lockAssetV1(
      new LockAssetV1Request({
        assetLockV1PB: {
          ...
          owner: { network: u1n1.organization, userId: u1n1.userId },
          hashInfo: {
            secret: hashSecret,
          },
          expirySecs: BigInt(45),
    ...
    const claimResult = await client.claimLockedAssetV1(
      new ClaimLockedAssetV1Request({
        assetLockClaimV1PB: {
          ...
          source: { network: u1n1.organization, userId: u1n1.userId },
          destination: { network: u2n1.organization, userId: u2n1.userId },
          sourceCertificate: srcCert,
          destCertificate: destCert,
          hashInfo: {
            secret: hashSecret,
