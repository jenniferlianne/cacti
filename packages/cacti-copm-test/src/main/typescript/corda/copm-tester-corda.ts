...
export class CopmTesterCorda implements CopmTester {
  ...
  constructor(log: Logger, networkMode: CopmNetworkMode) {
    ...
    this.copmCordaContainer = new CopmCordaContainer({
      ...
      remoteNetworkConfigJson: this.getRemoteNetworkConfig(),
    });
  }
  ...
  async startServer() {
    ...
    const copmPlugin = new PluginCopmCorda({
      instanceId: uuidV4(),
      logLevel: this.logLevel,
      containerPort: this.copmCordaContainer.apiPort,
    });
    ...
    this.log.info("api wrapper - starting");
    const { addressInfoGrpc, addressInfoCrpc } = await this.apiServer.start();
    this.log.info("api wrapper - started");
    const rpcPort = addressInfoGrpc.port;
    const rpcAddr = addressInfoGrpc.address;
  ...
  private getRPCConfig(): string {
    ...
    return configContent.replace(/127.0.0.1/g, "172.17.0.1");
  }

  private getRemoteNetworkConfig(): string {
    const configPath = path.resolve(
      __dirname,
      this.weaverRelativePath,
